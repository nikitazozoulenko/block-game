...
glm::vec3 WorldGenerator::unitSquareVectors[8] = { glm::vec3(1,1,1), glm::vec3(-1,1,1),  glm::vec3(1,-1,1),  glm::vec3(1,1,-1),
glm::vec3(-1,-1,1),  glm::vec3(1,-1,-1),  glm::vec3(-1,1,-1),  glm::vec3(-1,-1,-1) };

WorldGenerator::WorldGenerator() : weightX(5), weightY(991), weightZ(3167), weightFinal(7919), PERLIN_WEIGHT(40)
{

}

int WorldGenerator::GenSeedFromWorldPos(int& posX, int& posY, int& posZ)
{
...
glm::vec3 WorldGenerator::CreateRandomGridVector(int posX, int posY, int posZ)
{
...
float WorldGenerator::SimplexNoise(float x_in, float y_in, float z_in)
{
...
Chunk* WorldGenerator::GenChunkAtPosition(int chunkX, int chunkY, int chunkZ)
{
	...
	for (int z = 0; z != Z_CHUNK_SIZE; ++z)
	{
		for (int x = 0; x != X_CHUNK_SIZE; ++x)
		{
			...
			{
				if (SimplexNoise(chunkX + static_cast<float>(x) / X_CHUNK_SIZE, chunkY + static_cast<float>(y) / Y_CHUNK_SIZE, chunkZ + static_cast<float>(z) / Z_CHUNK_SIZE) < -0.1)
				{
